        ...
        if (tmp.length == 2) {
          ...
          mv(file.path, req.settings.imagesDir + newFileName, function(err) {
            if (err) {
              ...
              shouldReturnArray.push(newFileName);
              debug('shouldReturnArray.length', shouldReturnArray.length);
              debug('req.files.length', req.files.size);
              if(shouldReturnArray.length === Object.keys(req.files).length) {
                return res.status(200).json({ files: shouldReturnArray });
              }
            }
          });
        } else {
